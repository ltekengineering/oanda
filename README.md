...
- [Research](https://github.com/ltekengineering/oanda/blob/main/README.md#trading-api)
- [~Trading/Backtesting~]()
- [Account Monitoring](https://github.com/ltekengineering/oanda/blob/main/README.md#shiny-dashboard)
...
#### Prerequisites: Ensure that the following packages are installed
  ``` R
    ...
    > install.packages(c("curl","jsonlite","remotes","shiny","shinythemes","ini")) # package + dashboard
    > install.packages(c("curl","jsonlite","remotes","shiny","shinythemes","ini","zoo")) #package + dashboard + demo backtest & toy algo trading
  ```
#### Install oanda's api
  ``` R
    > remotes::install_github(repo="ltekengineering/oanda", subdir="oanda")
  ```
   If `remotes::install_github` fails then navigate to `repo/oanda`. Invoke R. Run the following command
   ``` R
